### **系统需求规格说明书 (SRS): Aetherius - 自主运维AI Agent (v1.6 最终版)**
...
| **执行网关 (Execution Gateway)** | 负责安全执行诊断命令的MCP代理组件 |
...
|----------|----------|--------------|----------|
| **SRE/运维工程师** | 负责系统监控和问题处理 | 接收诊断报告、审核建议、执行修复 | 完全访问 |
...
> 📋 **架构对应**: 此流程对应第5.1.1节整体系统架构图中的完整数据流路径
...
> 📋 **架构对应**: 详细实现见第5.5节多集群管理策略和第5.5.1节多集群架构图
...
> 📋 **实现对应**: 业务规则的技术实现详见第5.3节状态与历史管理和第6.2节任务管理模型
...
│                                输入层 (Input Layer)                           │
├─────────────────┬─────────────────┬─────────────────┬─────────────────────────┤
│   Alertmanager  │  K8s Events     │   External      │      Manual             │
│   Webhooks      │   Stream        │   Triggers      │      Triggers           │
│                 │   (Watch API)   │   (API/CLI)     │      (Web UI)           │
└─────────────────┴─────────────────┴─────────────────┴─────────────────────────┘
         │                 │                 │                       │
         └─────────────────┼─────────────────┼───────────────────────┘
                           │                 │
...
特殊规则:
- 如果同一问题在1小时内重复出现3次以上，优先级自动提升一级
- 涉及核心命名空间(kube-system, istio-system)的告警，优先级自动提升
- 多集群同时出现相同告警，优先级提升并触发关联分析
```
