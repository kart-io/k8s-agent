...
This package provides adapters to integrate the `pkg/agent/store` package with `common/options` configuration system.
...
Run tests with:
```bash
go test ./pkg/agent/store/adapters
```
...
### Before (Direct Usage)
```go
import "github.com/kart-io/k8s-agent/pkg/agent/store/redis"
...
import (
    ...
    "github.com/kart-io/k8s-agent/pkg/agent/store/adapters"
)
