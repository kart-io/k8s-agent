...
// run 执行 shell 命令
func (s *ShellTool) run(ctx context.Context, input *ToolInput) (*ToolOutput, error) {
	// 解析参数
	...
	if !ok || command == "" {
		...
		}, NewToolError(s.Name(), "invalid input", fmt.Errorf("command is required"))
	}
	...
	if !s.allowedCommands[command] {
		...
		}, NewToolError(s.Name(), "command not allowed", fmt.Errorf("command not in whitelist: %s", command))
	}
	...
	if !success {
		...
		}, NewToolError(s.Name(), "command execution failed", err)
	}
...
func (s *ShellTool) ExecuteScript(ctx context.Context, scriptPath string, args []string) (*ToolOutput, error) {
	return s.Invoke(ctx, &ToolInput{
...
func (s *ShellTool) ExecutePipeline(ctx context.Context, commands []string) (*ToolOutput, error) {
