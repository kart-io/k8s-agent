...
// run 执行计算
func (c *CalculatorTool) run(ctx context.Context, input *ToolInput) (*ToolOutput, error) {
	expression, ok := input.Args["expression"].(string)
	if !ok {
		return &ToolOutput{
			Success: false,
			Error:   "expression is required and must be a string",
		}, NewToolError(c.Name(), "invalid input", fmt.Errorf("expression is required"))
	}
	...
	if err != nil {
		return &ToolOutput{
			Success: false,
			Error:   err.Error(),
		}, NewToolError(c.Name(), "calculation failed", err)
	}
...
func (a *AdvancedCalculatorTool) run(ctx context.Context, input *ToolInput) (*ToolOutput, error) {
	...
	if !ok {
		...
		}, NewToolError(a.Name(), "invalid input", fmt.Errorf("operation is required"))
	}
	...
	if !ok {
		...
		}, NewToolError(a.Name(), "invalid input", fmt.Errorf("operands must be an array"))
	}
	...
	for i, op := range operands {
		...
		case string:
			...
			if err != nil {
				...
				}, NewToolError(a.Name(), "invalid operand", err)
			}
			nums[i] = f
		default:
			return &ToolOutput{
			...
			}, NewToolError(a.Name(), "invalid operand type", fmt.Errorf("invalid type: %T", v))
		}
	}
	...
	case "subtract":
		if len(nums) < 2 {
			return &ToolOutput{Success: false, Error: "subtract requires at least 2 operands"}, nil
		}
		result = nums[0]
	...
	case "divide":
		if len(nums) < 2 {
			return &ToolOutput{Success: false, Error: "divide requires at least 2 operands"}, nil
		}
		result = nums[0]
		for _, n := range nums[1:] {
			if n == 0 {
				return &ToolOutput{Success: false, Error: "division by zero"}, nil
			}
			result /= n
		}
	case "power":
		if len(nums) != 2 {
			return &ToolOutput{Success: false, Error: "power requires exactly 2 operands"}, nil
		}
		result = math.Pow(nums[0], nums[1])
	case "sqrt":
		if len(nums) != 1 {
			return &ToolOutput{Success: false, Error: "sqrt requires exactly 1 operand"}, nil
		}
		result = math.Sqrt(nums[0])
	case "abs":
		if len(nums) != 1 {
			return &ToolOutput{Success: false, Error: "abs requires exactly 1 operand"}, nil
		}
		result = math.Abs(nums[0])
	case "sin":
		if len(nums) != 1 {
			return &ToolOutput{Success: false, Error: "sin requires exactly 1 operand"}, nil
		}
		result = math.Sin(nums[0])
	case "cos":
		if len(nums) != 1 {
			return &ToolOutput{Success: false, Error: "cos requires exactly 1 operand"}, nil
		}
		result = math.Cos(nums[0])
	case "tan":
		if len(nums) != 1 {
			return &ToolOutput{Success: false, Error: "tan requires exactly 1 operand"}, nil
		}
		result = math.Tan(nums[0])
	case "log":
		if len(nums) != 1 {
			return &ToolOutput{Success: false, Error: "log requires exactly 1 operand"}, nil
		}
		result = math.Log10(nums[0])
	case "ln":
		if len(nums) != 1 {
			return &ToolOutput{Success: false, Error: "ln requires exactly 1 operand"}, nil
		}
	...
	default:
		...
		}, NewToolError(a.Name(), "unknown operation", fmt.Errorf("unknown operation: %s", operation))
	}
