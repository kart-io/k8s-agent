...
// run 执行搜索
func (s *SearchTool) run(ctx context.Context, input *ToolInput) (*ToolOutput, error) {
	query, ok := input.Args["query"].(string)
	if !ok || query == "" {
		return &ToolOutput{
			Success: false,
			Error:   "query is required and must be a non-empty string",
		}, NewToolError(s.Name(), "invalid input", fmt.Errorf("query is required"))
	}
	...
	if err != nil {
		return &ToolOutput{
			Success: false,
			Error:   err.Error(),
		}, NewToolError(s.Name(), "search failed", err)
	}
