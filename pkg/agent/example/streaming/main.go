...
// 示例 5: 流式转换和过滤
func example5StreamTransform() {
	fmt.Println("--- 示例 5: 流式转换和过滤 ---")
	...
	go func() {
		...
		for i := 1; i <= 10; i++ {
			input <- stream.NewStreamChunk(i)
		}
	}()

	// 转换：将数字乘以 2
	transformed := manager.Transform(ctx, input, func(chunk *stream.StreamChunk) (*stream.StreamChunk, error) {
		if num, ok := chunk.Data.(int); ok {
			return stream.NewStreamChunk(num * 2), nil
		}
		return chunk, nil
	})

	// 过滤：只保留偶数
	filtered := manager.Filter(ctx, transformed, func(chunk *stream.StreamChunk) bool {
		if num, ok := chunk.Data.(int); ok {
			return num%4 == 0 // 2的倍数再乘以2，只保留4的倍数
		}
		return false
	})
...
// 示例 6: 流式统计
func example6StreamStats() {
	fmt.Println("--- 示例 6: 流式统计 ---")

	_ = context.Background() // 用于未来扩展

	// 创建统计对象
	stats := stream.NewStreamStats()
	...
	go func() {
		...
		for i := 1; i <= 100; i++ {
			chunk := stream.NewStreamChunk(fmt.Sprintf("Data %d", i))
			input <- chunk
	...
	for chunk := range input {
		stats.RecordChunk(int64(len(fmt.Sprintf("%v", chunk.Data))))
